...
class RotationMatrixBuilderTfGramSchmidt(object):
    ...
    def create(self):
        #return tf.eye(self.dim)
        epsilon = 1e-8
        vectors = tf.get_variable('unorthogonal_rotation', [self.dim, self.dim], dtype=tf.float32, initializer=lambda shape, dtype, *args, **kwargs: tf.eye(shape[0], shape[1], dtype=dtype))
        # add batch dimension for matmul
        basis = tf.expand_dims(vectors[0, :] / (tf.norm(vectors[0, :]) + epsilon), 0)
        for i in range(1,vectors.get_shape()[0].value):
            ...
            basis = tf.concat([basis, w / (tf.norm(w) + epsilon)],axis=0)
        return basis

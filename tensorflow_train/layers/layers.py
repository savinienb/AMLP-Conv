...
def Amlp_Conv(inputs,
      ...
      N, z_0,D_0, H_0, W_0  =  inputs.get_shape().as_list() #tf.shape(u)
      z = np.product(filters)
      att_n = 1
      ones_kernel = tf.ones([1,1,1,1])
      kernel_2 = he_initializer
      shape = (N,z_0,H_0,W_0)
      k = 3


      in_H = tf.layers.conv3d(inputs ,filters=z_0, kernel_size=1, strides=[1]*3,trainable = is_training, padding = "VALID",kernel_initializer=kernel_initializer,activation = tf.nn.relu,data_format='channels_first')
      in_D = tf.layers.conv3d(inputs ,filters=z_0, kernel_size=1, strides=[1]*3,trainable = is_training, padding = "VALID",kernel_initializer=kernel_initializer,activation = tf.nn.relu,data_format='channels_first')
      in_W = tf.layers.conv3d(inputs ,filters=z_0, kernel_size=1, strides=[1]*3,trainable = is_training, padding = "VALID",kernel_initializer=kernel_initializer,activation = tf.nn.relu,data_format='channels_first')

      in_D =tf.reduce_sum(in_D,axis=2,keepdims=True)
      in_H =tf.reduce_sum(in_H,axis=3,keepdims=True)
      in_W =tf.reduce_sum(in_W,axis=4,keepdims=True)

      in_H = tf.layers.conv3d(in_H ,filters=z_0,kernel_size=1, strides=[1]*3, trainable = is_training,padding = "VALID",kernel_initializer=kernel_initializer,activation = None,data_format='channels_first')
      in_W = tf.layers.conv3d(in_W ,filters=z_0,kernel_size=1, strides=[1]*3,trainable = is_training, padding = "VALID",kernel_initializer=kernel_initializer,activation = None,data_format='channels_first')
      in_D = tf.layers.conv3d(in_D ,filters=z_0,kernel_size=1, strides=[1]*3,trainable = is_training, padding = "VALID",kernel_initializer=kernel_initializer,activation = None,data_format='channels_first')

      attention = in_H + in_W + in_D
      ini_s = inputs * tf.nn.sigmoid(attention)

      o = tf.layers.conv3d(ini_s,filters=z,kernel_size=kernel_size, strides=[1]*3, trainable = is_training,padding = "SAME",kernel_initializer=kernel_initializer,activation = tf.nn.relu,data_format='channels_first')

      ini_s2 = tf.reshape(o,(N,z,-1))
      ini_s2 = layer_norm(ini_s2,axis=-1)
      ini_s2 = tf.reshape(ini_s2,(N,z,D_0,H_0,W_0))

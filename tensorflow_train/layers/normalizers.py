...
def instance_norm(inputs, is_training, name='', data_format='channels_first', epsilon=1e-5, use_offset=True, use_scale=True, beta_initializer=tf.constant_initializer(0.0), gamma_initializer=tf.constant_initializer(1.0)):
    with tf.variable_scope(name):
        ...
        if use_offset:
            ...
            offset = tf.zeros([depth], tf.float32, name='offset')
        if use_scale:
            scale = tf.get_variable('scale', [depth], trainable=is_training, initializer=gamma_initializer)
        else:
            scale = tf.ones([depth], tf.float32, name='scale')

...
def upsample_interpolation_function(inputs, factors, interpolation_function, support, name, data_format='channels_first', padding='same', output_size_cropped=None):
    ...
    with tf.name_scope(name):
        ...
        for i in range(len(split_inputs)):
            if dim == 2:
                current_output = tf.nn.conv2d_transpose(split_inputs[i], kernel_tensor, output_shape, strides, data_format=data_format_tf, name='conv' + str(i), padding=padding)
            else: # dim == 3
                current_output = tf.nn.conv3d_transpose(split_inputs[i], kernel_tensor, output_shape, strides, data_format=data_format_tf, name='conv' + str(i), padding=padding)
        ...
        if cropping:
            ...
            output_size_cropped = output_size_cropped or [input_size[i] * factors[i] for i in range(dim)]
            output_shape = get_tensor_shape(batch_size=inputs_shape[0], channel_size=num_inputs, image_size=output_size_cropped, data_format=data_format)
            outputs = tf.slice(outputs, paddings, output_shape)
...
def upsample_linear(inputs, factors, *args, **kwargs):
...
def upsample_cubic(inputs, factors, *args, **kwargs):
    return upsample_interpolation_function(inputs=inputs,
                                           factors=factors,
...
def upsample_lanczos(inputs, factors, order=4, *args, **kwargs):
...
def upsample2d_linear(inputs, factors, *args, **kwargs):
    check_2d(inputs, factors)
    return upsample_linear(inputs, factors, *args, **kwargs)


def upsample2d_cubic(inputs, factors, *args, **kwargs):
    check_2d(inputs, factors)
    return upsample_cubic(inputs, factors, *args, **kwargs)


def upsample2d_lanczos(inputs, factors, *args, **kwargs):
    check_2d(inputs, factors)
    return upsample_lanczos(inputs, factors, *args, **kwargs)


def upsample3d_linear(inputs, factors, *args, **kwargs):
    check_3d(inputs, factors)
    return upsample_linear(inputs, factors, *args, **kwargs)


def upsample3d_cubic(inputs, factors, *args, **kwargs):
    check_3d(inputs, factors)
    return upsample_cubic(inputs, factors, *args, **kwargs)


def upsample3d_lanczos(inputs, factors, *args, **kwargs):
    check_3d(inputs, factors)
    return upsample_lanczos(inputs, factors, *args, **kwargs)

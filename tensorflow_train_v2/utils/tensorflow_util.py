...
def get_variables_as_tensors(variables):
    """
    Return a list of variable values as a list of Tensors.
    :param variables: List of variables.
    :return: var_values list
    """
    # use tf.constant to make sure that the variable is not returned as a view, but copied
    return [tf.constant(v.value()) for v in variables]


def get_variables_ema_as_tensors(variables, ema):
    """
    Return a list of emas of variable values as a list of Tensors.
    :param variables: List of variables.
    :param ema: tf.train.ExponentialMovingAverage that encapsulates the variables.
    :return: var_ema_values list
    """
    # use tf.constant to make sure that the variable is not returned as a view, but copied
    return [tf.constant(ema.average(v).value()) for v in variables]


def get_variable_and_ema_values(variables, ema):
    """
    Return a list of variable values and averages from a corresponding EMA object as lists of Tensors.
    :param variables: List of variables.
    :param ema: tf.train.ExponentialMovingAverage that encapsulates the variables.
    :return: model_var_values, model_var_ema_values
    """
    return get_variables_as_tensors(variables), get_variables_ema_as_tensors(variables, ema)


def set_variable_values(variables, var_values):
    """
    Set the variables to the given values.
    :param variables: List of variables.
    :param var_values: List of variable values as tensors. Must be corresponding to model.trainable_weights.
    """
    for v, value in zip(variables, var_values):
        v.assign(value)

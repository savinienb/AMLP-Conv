...
class LossMetricLogger(object):
    ...
    def print(self, current_iteration, summary_values, learning_rate=None):
        """
        Print the summary for the given current iteration and summary values.
        :param current_iteration: The current iteration.
        :param learning_rate: The optional learning rate.
        :param summary_values: The current calculated summary values. Keys must be the same as self.summary_placeholders_dict.keys()
        """
        date_string = self.now.strftime('%H:%M:%S')
        print_string = f'{date_string}: {self.name} iter: {current_iteration} '
        if learning_rate:
            print_string += f'lr: {learning_rate:.2e} '
        for key, value in summary_values.items():
            value_string = self.print_format.format(value) if self.print_format is not None else str(value)
            print_string += f'{key}: {value_string} '
        print_string += f'seconds: {self.time_since_last_finalize.seconds}.{self.time_since_last_finalize.microseconds // 1000:03d}'
        print(print_string)

    def finalize(self, current_iteration, learning_rate=None):
        """
        ...
        self.print(int(current_iteration), summary_values, learning_rate=learning_rate)

        self.reset_metrics()

        return summary_values

    def finalize_with_optimizer(self, optimizer):
        """
        Finalize the summary fo the current iteration. Writes summary, .csv file, and prints a short summary string. Additionally resets the internal times and the losses' running mean.
        :param optimizer: The optimizer.
        :return: Dictionary of all current summary and loss values.
        """
        # take the iteration *before* call of finalizer -> -1
        current_iteration = int(optimizer.iterations) - 1
        learning_rate = optimizer.learning_rate
        if callable(learning_rate):
            learning_rate = learning_rate(current_iteration)
        learning_rate = float(learning_rate)
        return self.finalize(current_iteration, learning_rate)
